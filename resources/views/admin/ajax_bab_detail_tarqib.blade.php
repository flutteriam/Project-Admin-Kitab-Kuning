                    ...
                    <div class="m-l-10 m-t-10 p-10" style="width: 100%; border: 2px dashed #c4c4c4;">
                        <div class="d-flex flex-row-reverse flex-wrap">
                            @foreach ($bait->kata as $kata)
                                <div class="badge m-1 float-right kata" style="position: relative; border-radius: 30px;padding: 10px; background-color: #fff6e3; cursor: pointer;" onclick="editData({{ $kata->id }}, this)">
                                    ...
                                    @if (is_null($kata->tarqib))
                                        <h5><span class="badge badge-primary mt-2">Set</span></h5>
                                    @endif

                                    @if (!is_null($kata->tarqib) && $kata->tarqib->progress < 100)
                                        <div class="progress">
                                            <div class="progress-bar bg-success" role="progressbar" style="width: {{$kata->tarqib->progress}}%" aria-valuenow="{{$kata->tarqib->progress}}" aria-valuemin="0" aria-valuemax="100"></div>
                                        </div>
                                    @endif

                                    @if (!is_null($kata->tarqib) && $kata->tarqib->progress == 100)
                                        <div class="ribbon ribbon-success ribbon-vertical-right" style="right: 0;height: 25px;top: -5px;line-height: 25px;">
