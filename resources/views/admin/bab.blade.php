...
@section('content')
<div id="context-menu">
    <div class="item" id="btn-delete-word" data-kata_id="" onclick="deleteDataWord(this)"><i class="fa fa-trash"></i> Hapus</div>
    <div class="item" id="btn-duplicate-word" onclick="btnDuplicate(this)" data-id=""><i class="fa fa-copy"></i> Duplikat</div>
    <div class="item" id="btn-edit-word" onclick="editDataWord(this)" data-id=""><i class="fa fa-edit"></i> Edit</div>
</div>
                    ...
                    <div class="col-md-4">
                        <select name="category" id="category" autocomplete="off" class="form-control">
                            ...
                            @foreach ($categories as $key => $value)
                                <option value="{{ $value->id }}" {{ (isset($active_post) && $active_post->category_id == $value->id) ? "selected" : '' }}>{{ $value->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        <select name="post" id="post" autocomplete="off" class="form-control">
                            ...
                            @if ($active_post)
                                @foreach ($all_post as $value)
                                    <option value="{{ $value->id }}" {{ $active_post->id == $value->id ? 'selected' : '' }}>{{ $value->title }}</option>
                                @endforeach
                            @endif
                        </select>
                    </div>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form id="form-data" method="POST">
                ...
                <input type="hidden" name="post_id" id="inputPost" value="{{ $active_post != null ? $active_post->id : '' }}">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <textarea type="text" placeholder="Terjemahan Judul" name="translate_title" id="fieldTransTitle" class="form-control"></textarea>
                    </div>
                </div>
        ...
        <div class="modal-content">
            ...
            <form id="form-data-bait" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <input type="hidden" name="post_id" id="fieldPost" value="">
        ...
        <div class="modal-content">
            ...
            <form id="form-data-word" method="POST">
                ...
                <div class="modal-body">
                    ...
                    <input type="hidden" name="post_id" id="fieldPostWord">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-danger m-r-10" id="btn-del-word" data-kata_id="" onclick="deleteDataWord(this)">Hapus</button>
                </div>
            </form>
        </div>
    </div>
        ...
        new Promise((resolve, reject) => {
            ...
            $axios.get(`${url}`)
                .then(({data}) => {
                    ...
                    // get_post(bab.post.category_id, 'fieldPost').then(() => $("#fieldPost").val(bab.post_id))
                    $("#inputCategory").val(bab.post.category_id)
                    $("#inputPost").val(bab.post_id)
...
<script>
        ...
        new Promise((resolve, reject) => {
            ...
            $axios.get(`${url}`)
                .then(({data}) => {
                    ...
                    $("#fieldPostWord").val(chapter.post_id)
    ...
    const saveEditBait = (element) => {
        let { id, babId, postId } = $(element).data();
        const FD = new FormData();
